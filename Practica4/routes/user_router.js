...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!jwtSecret) {
      logger.error('JWT secret is not configured. Set SECRET_KEY or JWT_SECRET in the environment.');
    ...
    try {
      ...
      logger.error('JWT signing failed:', signErr.message || signErr);
...
router.post('/registro', async (req, res) => {  // --> POST
  ...
  try {
    ...
    try {
      ...
      logger.error('JWT signing failed after registration:', signErr.message || signErr);
      return res.redirect('/');
    }
    ...
    logger.error('Registro fallido:', err.message || err);
